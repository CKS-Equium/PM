---
name: smart-status
description: Intelligent workflow hub providing comprehensive repository overview and quick branch navigation. Use to check status, switch branches, or start your day.
---
...
**Dependencies:** Optional `gh` CLI (GitHub CLI) for viewing open pull requests.
...
#### 1.1 Fetch Remote State (ALWAYS DO THIS FIRST)

```bash
git fetch origin
```

**This must run before anything else** so that all remote branches (including those pushed by collaborators) are visible locally. Without this, `git branch -r` will be stale and collaborators won't see each other's work.

#### 1.2 Branch Information
...
**Note:** fetch already happened in 1.1. The `2>/dev/null` handles branches with no upstream set.
...
#### 2.1 List All Branches (Local + Remote)
...
**Important:** Show both local branches AND remote-only branches (branches pushed by collaborators that don't exist locally yet). Compare `git branch` with `git branch -r` to identify remote-only branches.
...
🌐 REMOTE-ONLY BRANCHES (not checked out locally):
  → origin/feat/new-feature-by-colleague
      Last: 3 hours ago - "feat: add new component"
      To checkout: git checkout feat/new-feature-by-colleague
```

**Note:** Remote-only branches are those in `git branch -r` that have no corresponding local branch. Always show these so collaborators can see each other's work.
